      ...
      <Flex h="calc(100vh - 64px)" bg="bg.deep">
        ...
        >
          ...
          <VStack gap={2} align="stretch" overflowY="auto" maxH="calc(100% - 100px)">
            <Text color="text.mist" fontSize="xs" fontWeight="bold" letterSpacing="widest" mb={2}>RECENT HISTORY</Text>
            {conversations.map((conv) => (
              ...
              >
                <HStack gap={3}>
                  <MessageSquare size={16} color={selectedConvId === conv.id ? '#00FF41' : '#888888'} />
                  <Text color="text.neon" fontSize="sm" truncate>{conv.title || 'Untitled Chat'}</Text>
                </HStack>
              </Box>
            ))}
          </VStack>
          ...
          <Box flex={1} overflowY="auto" p={6} pb="100px">
            <VStack gap={6} align="stretch">
              {messages.length === 0 && !isStreaming && (
                <VStack pt={20} gap={4}>
                  <Heading color="brand.matrix" size="xl" fontFamily="heading" textShadow="0 0 10px currentColor">
                    NEO-LOG AI
                  </Heading>
                  <Text color="text.mist" fontFamily="mono">[ SYSTEM IDLE. WAITING FOR INPUT... ]</Text>
                </VStack>
              )}
              ...
              {isStreaming && (
                <HStack align="start" gap={4} maxW="800px" mx="auto" w="full">
                  ...
                  <VStack align="start" gap={1} flex={1}>
                    <Text fontSize="xs" color="text.mist" fontWeight="bold" fontFamily="mono">NEO-LOG AI</Text>
                    <Box 
                      ...
                      w="full"
                    >
                      <Text whiteSpace="pre-wrap">{streamingContent}<Box as="span" animation="pulse 1s infinite">▌</Box></Text>
                    </Box>
                  </VStack>
                </HStack>
              )}
          ...
          >
            <Box maxW="800px" mx="auto" w="full" position="relative">
              <HStack 
                ...
                gap={2}
              >
                <IconButton aria-label="Upload image" variant="ghost" size="sm" color="text.mist" _hover={{ color: 'brand.cyber' }}>
                  <Image size={20} />
                </IconButton>
                <IconButton aria-label="Voice input" variant="ghost" size="sm" color="text.mist" _hover={{ color: 'brand.cyber' }}>
                  <Mic size={20} />
                </IconButton>
                <Input
                  variant="subtle"
