import { pgTable, uuid, text, timestamp, boolean, jsonb, integer, time, pgPolicy } from 'drizzle-orm/pg-core';
import { sql } from 'drizzle-orm';

// User Settings Table
export const neologUserSettings = pgTable('neolog_user_settings', {
  ...
  font_size: text('font_size', { enum: ['small', 'medium', 'large', 'xlarge'] }).default('medium'),
  ...
  date_format: text('date_format', { enum: ['YYYY-MM-DD', 'MM/DD/YYYY', 'DD/MM/YYYY'] }).default('YYYY-MM-DD'),
...
export const neologBehaviorDefinitions = pgTable('neolog_behavior_definitions', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id'), // References auth.users
  name: text('name').notNull(),
  category: text('category', { enum: ['health', 'expense', 'income', 'habit', 'other'] }).notNull(),
  icon: text('icon'),
...
export const neologBehaviorRecords = pgTable('neolog_behavior_records', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull(), // References auth.users
  definition_id: uuid('definition_id').references(() => neologBehaviorDefinitions.id, { onDelete: 'cascade' }).notNull(),
...
// Goals Table
export const neologGoals = pgTable('neolog_goals', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull(), // References auth.users
  name: text('name').notNull(),
...
// Notes Table
export const neologNotes = pgTable('neolog_notes', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull(), // References auth.users
  title: text('title'),
...
// Conversations Table
export const neologConversations = pgTable('neolog_conversations', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull(), // References auth.users
  title: text('title'),
  created_at: timestamp('created_at', { withTimezone: true, mode: 'string' }).defaultNow(),
...
// Messages Table
export const neologMessages = pgTable('neolog_messages', {
  id: uuid('id').primaryKey().defaultRandom(),
  conversation_id: uuid('conversation_id').references(() => neologConversations.id, { onDelete: 'cascade' }).notNull(),
  user_id: uuid('user_id').notNull(), // References auth.users
