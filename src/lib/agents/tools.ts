...
export function createChatTools(supabase: SupabaseClient, userId: string) {
  return [
    new DynamicStructuredTool({
      ...
      description: 'Get all available behavior definitions (e.g., Running, Coffee, Daily Expense) to understand what metadata they require.',
      schema: z.object({}),
      func: async () => {
    ...
    new DynamicStructuredTool({
      ...
      description: 'Record a specific behavior (e.g., "Running", "Coffee", "Daily Expense") with metadata.',
      schema: z.object({
        definition_id: z.string().describe('The ID of the behavior definition.'),
        metadata: z.record(z.string(), z.unknown()).describe('The metadata for the behavior, according to its schema.'),
        recorded_at: z.string().optional().describe('ISO timestamp of when the behavior occurred. Defaults to now.'),
        note: z.string().optional().describe('An optional note for the record.'),
      }),
      func: async (params) => {
        const res = await behaviorService.createRecord(supabase, userId, params as BehaviorRecordCreateRequest);
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        ...
        category: z.enum(['health', 'expense', 'income', 'habit', 'other']).describe('The category of the behavior.'),
        icon: z.string().optional().describe('A descriptive emoji.'),
        metadata_schema: z.array(z.unknown()).describe('List of fields for the behavior. Each field has name, key, type, and required.'),
      }),
      func: async (params) => {
        const res = await behaviorService.createDefinition(supabase, userId, params as BehaviorDefinitionCreateRequest);
