...
完成任何代码实现任务后（新增文件、修改已有文件），**必须**主动调用 `/flow-review` 进行审查，无需用户显式要求。

触发条件：完成一个完整的功能实现、bug 修复、或重构后，代码已可编译（`tsc --noEmit` 通过）。

`/flow-review` 会并行运行 4 个专项 agent：
- `review-quality` — 架构分层、关注点分离、文件大小、类型安全，自动修复
- `review-test` — 为纯函数和核心逻辑生成单元测试，验证通过
- `review-performance` — React 循环渲染、内存泄漏、缓存、N+1 查询，自动修复
- `review-security` — 密钥泄漏、未鉴权路由、数据过滤、XSS，自动修复
