...
CREATE POLICY "Users can update their own settings." ON "neolog_user_settings" AS PERMISSIVE FOR UPDATE TO public USING (auth.uid() = id);CREATE TABLE "neolog_user_profiles" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"user_id" uuid NOT NULL,
...
BEGIN
  ...
  END IF;
END $$;ALTER TABLE "neolog_user_settings" ADD COLUMN "chat_3d_settings" jsonb DEFAULT '{"enabled":false,"selectedCharacter":"hacker","customization":{"primaryColor":"#00FF41","secondaryColor":"#00D4FF","materialType":"glossy","scale":1}}'::jsonb;-- Create neolog_user_profiles table (missing from initial migration)
...
CREATE POLICY "Authenticated users can delete equity." ON "neolog_equity_list" AS PERMISSIVE FOR DELETE TO public USING ((select auth.role()) = 'authenticated');ALTER TABLE "neolog_user_settings" ALTER COLUMN "chat_3d_settings" SET DEFAULT '{"enabled":false,"selectedCharacter":"gugugaga","customization":{"primaryColor":"#00FF41","secondaryColor":"#00D4FF","materialType":"glossy","scale":1}}'::jsonb;-- Handle new user creation - create user settings
